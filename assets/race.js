...
// ── 월즈 진출 (2026) ─────────────────────────────────────────────
// ⚠ 여기 값은 **공식 규정과 이미 끝난 결과**다. 코드가 추론한 게 아니라 사람이 확인해
//   적어 둔 사실이므로, 시즌이 바뀌면 사람이 고쳐야 한다.
//   대신 "그래서 지금 누가 확정됐나"는 **절대 손으로 적지 않는다** — 그건 아래 함수가
//   경우의 수 엔진이 계산한 플레이오프 확정 여부에서 그대로 끌어낸다.
//   (손으로 적으면 결과가 바뀌어도 화면은 옛말을 계속하게 된다)
//
// 자주 헷갈리는 것: 월즈행은 **정규 라운드 순위가 아니라 플레이오프 최종 순위**로 정해진다.
//   정규 라운드 순위는 플레이오프의 **출발 위치**만 정한다.
const WORLDS = {
  year: 2026,
  seeds: 4,            // LCK 시드 4장 (원래 3장 + MSI 우승 지역 특전 1장), 전원 스위스 스테이지 직행
  cutRank: 4,          // 월즈행 = 플레이오프 최종 4위 안 → 패자조 3라운드를 이겨야 한다
  msiChampion: "hle",  // 2026 MSI 우승 (7/12 결승 vs BLG)
  // MSI 우승팀 특전: 플레이오프 진출이 확정되는 즉시 월즈 진출 + 최소 4시드
  msiPerk: "MSI 우승팀 특전 — 플레이오프 진출이 확정되는 순간 월즈 진출과 최소 4시드가 함께 확정",
};

/** 지금 월즈 진출이 **확정된** 팀. 확정 근거가 없으면 빈 배열 (억지로 만들지 않는다). */
function worldsLocked() {
  const t = WORLDS.msiChampion;
  const r = typeof raceCached === "function" ? raceCached("r34L") : null;
  if (!r || !TEAM_MAP[t]) return [];
  // "플레이오프 직행" 컷(4위 안)이 lock 이어야 한다. 컷 정의가 바뀌어도 k 로 찾으므로 안전하다.
  const ci = r.cuts.findIndex(c => c.k === 4);
  if (ci < 0 || !r.teamOrder.includes(t)) return [];
  if (r.stateNow(t, ci) !== "lock") return [];
  return [{ team: t, seed: WORLDS.seeds, why: WORLDS.msiPerk }];
}
