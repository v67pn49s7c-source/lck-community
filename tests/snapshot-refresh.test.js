...
// 새 스키마는 v3만 사용하고, 이미 방문한 브라우저의 v1·v2는 둘 다 즉시 폐기해야 한다.
const snapStart = source.indexOf('const SNAP_KEY = "nexus_snap_v3";');
const snapEnd = source.indexOf("\nfunction snapshotSave", snapStart);
assert(snapStart >= 0 && snapEnd > snapStart, "v3 스냅샷 초기화 블록을 찾을 수 없음");
...
assert.strictEqual(snapContext.__snapKey, "nexus_snap_v3");
assert.deepStrictEqual(removed, ["nexus_snap_v1", "nexus_snap_v2"]);
