...
assert.strictEqual(Object.keys(_test.DEFAULT_SOCIAL).length, 10, "10개 LCK 팀의 공식 Instagram 기본 연결이 필요함");
assert.strictEqual(_test.DEFAULT_SOCIAL.t1.instagram, "t1lol");
assert.strictEqual(_test.DEFAULT_SOCIAL.dns.instagram, "soopers_lol");
assert.deepStrictEqual(_test.accountLinks("UC-test", { instagram: "@team.official", x: "TeamX" }), [
  { platform: "youtube", label: "YouTube", url: "https://www.youtube.com/channel/UC-test" },
  { platform: "instagram", label: "Instagram", handle: "@team.official", url: "https://www.instagram.com/team.official/" },
  { platform: "x", label: "X", handle: "@TeamX", url: "https://x.com/TeamX" },
]);
...
assert(teamHtml.includes('id="content-prev"') && teamHtml.includes('id="content-next"'),
  "헤더에 이전·다음 버튼이 있어야 함");
assert(teamHtml.includes('id="content-accounts"'), "팀 공식 계정 바로가기 영역이 있어야 함");
assert(app.includes("renderTeamContent") && app.includes("CONTENT_PLATFORM_NAME"),
  "통합 콘텐츠 캐러셀 렌더러가 있어야 함");
assert(app.includes("j.accounts") && app.includes("content-account"),
  "콘텐츠 API가 실패해도 공식 계정 링크는 보여야 함");
...
assert(api.includes("DEFAULT_SOCIAL") && api.includes("accountLinks"),
  "10개 팀 공식 계정 기본값과 직접 이동 링크를 제공해야 함");
