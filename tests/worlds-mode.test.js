...
assert((app.match(/location\.replace\("worlds\.html"\)/g) || []).length === 2,
  "루트 홈은 즉시 판정과 최신 경기 수신 뒤 모두 worlds.html 전환을 확인해야 함");
assert(/data-tab="season-mode"/.test(admin) && /value="auto"/.test(admin)
  && /value="lck"/.test(admin) && /value="worlds"/.test(admin), "관리자 전환·복귀 스위치");
assert(/worlds\.html\?preview=1/.test(admin), "공개 전 월즈 홈 미리보기 링크");
...
console.log("worlds-mode.test: 17 통과");
